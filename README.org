# -*- mode: org; -*- important: ne pas enlever cette ligne
...
#+begin_src shell
  podman run -it \
         --name testgitlocal \
         -v /var/lib/gitea/data/gitea-repositories:/repos:ro \
         fb5e48a0afc7 /bin/bash
#+end_src
...
ATTENTION, le runner a besoin d'une option particulière dans le cas où
il est lancé en mode =exec= :

#+begin_src shell
export DOCKER_HOST=unix:///run/user/1000/podman/podman.sock
/home/michel/dev/gitea/act_runner-0.2.13-linux-amd64 exec -c container/gitea-act_runner/data/config.yml --container-daemon-socket=$DOCKER_HOST
#+end_src

En mode =daemon=, cette option n'est pas reconnue, mais la
configuration appropriée se fait dans le fichier de config.
